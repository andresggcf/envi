...
public class CubemanController : MonoBehaviour
{
    ...
    public void Scroll_change_smooth (float smoothVal)
    {
        smooth = smoothVal;   
        textoSmooth.GetComponent<Text>().text = ((float)smoothVal).ToString();
    ...
    void Update()
    {
        ...
        //posPointMan.z = posPointMan.z;

        // store the initial position
        if (initialPosUserID != playerID)
        {
            initialPosUserID = playerID;
            initialPosOffset = transform.position - (new Vector3(-posPointMan.x, 0, posPointMan.z));
        }

        transform.position = initialPosOffset + (new Vector3(-posPointMan.x, 0, posPointMan.z));
        // Funcion que actualiza la posici?n de los huesos
        for (int i = 0; i < bones.Length; i++)
        {
            if (bones[i] != null)
            {
            ...
            for (int i = 0; i < bones.Length; i++)
            {
                ...
                if (bones[i] != null)
                {
                    if (bones[i].gameObject.activeSelf)
                    {
                        ...
                        {
                            //abril 12 - Se cambia true para evitar el renderizado de las lineas del cuerpo
                            lines[i].gameObject.SetActive(false);
        ...
        //Abril 12 - Despues
        Quaternion nuevoQ = Head.transform.rotation;
        nuevoQ.eulerAngles = new Vector3(nuevoQ.eulerAngles.x, nuevoQ.eulerAngles.y, 0);
        Head.transform.rotation = nuevoQ;
        Camera.main.transform.rotation = Quaternion.Slerp(Camera.main.transform.rotation, Head.transform.rotation, Time.deltaTime * smooth);
        Camera.main.transform.position = Vector3.Lerp(Camera.main.transform.position, Head.transform.position, Time.deltaTime * smooth);
    }
